...
class DeciphermentEngine:
    ...
    def find_slot_omega_candidates(self):
        ...
        return (
            ...
            .agg(
                ...
                sample_tokens=("clean", lambda values: ", ".join(pd.unique(values)[:5])),
                sections=("section", lambda values: ", ".join(pd.unique(values))),
                folios=("folio", lambda values: ", ".join(pd.unique(values)[:8])),
            )
    ...
    def run_folio_holdout(self, holdout_frac=0.2, seed=42, min_occurrences=5):
        ...
        train_pmi = DeciphermentEngine(train_df).compute_carrier_excess_specificity(
            min_occurrences=min_occurrences
        )
        test_pmi = DeciphermentEngine(test_df).compute_carrier_excess_specificity(
            min_occurrences=max(2, min_occurrences // 2)
        )
